(() => {
  ...
  async function load(){
    try {
      ...
      hosts.innerHTML = rows.length ? rows.map(host => `<div class="host"><a href="${esc(host.url)}" rel="noopener noreferrer">${esc(host.hostname)}</a><span class="badge ${esc(host.state || 'unknown')}">${esc(label[host.state] || label.unknown)}</span></div>`).join('') : '<div class="meta">Chưa có hostname công khai để hiển thị.</div>';
      updated.textContent = `Cập nhật lần cuối: ${new Date(data.updated_at).toLocaleString('vi-VN')}. Làm mới tự động mỗi 60 giây.`;
    } catch (_) { state.textContent = 'Chưa rõ'; meta.textContent = 'Trang trạng thái tạm thời chưa đọc được dữ liệu.'; setBadge(badge, 'unknown'); hosts.innerHTML = '<div class="meta">Vui lòng thử lại sau.</div>'; }
  }
