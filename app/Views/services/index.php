...
<div class="page-head service-pro-head"><div><p class="eyebrow">TMS OS V14 · UNIFIED SYSTEM CORE</p><h1>Unified System Core</h1><p>Một nguồn trạng thái thống nhất cho Service Manager, Guardian, Diagnostics, Terminal và Dashboard.</p></div><div class="head-actions"><button type="button" class="btn btn-secondary" data-service-refresh>Làm mới</button><form method="post" action="/services/restart-all" data-confirm="Khởi động lại toàn bộ dịch vụ đã cài?" data-action-form><input type="hidden" name="csrf" value="<?=tms_h($csrf)?>"><button class="btn btn-primary">Restart All</button></form></div></div>
