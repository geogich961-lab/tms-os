<?php
$title='Internet Access Center · TMS OS';$showShell=true;require dirname(__DIR__).'/layouts/header.php';
$state=(string)($status['status']??'stopped');
...
<section class="panel-card"><div class="section-title-row"><div><p class="eyebrow">ADVANCED</p><h2>Ngrok & TMS Relay</h2></div></div><p class="muted">Ngrok cần token. TMS Relay cần VPS có SSH key và reverse proxy trỏ tới cổng remote.</p>
...
<section class="panel-card"><div class="section-title-row"><h2>Nhật ký kết nối</h2><button type="button" class="btn btn-secondary btn-small" id="cf-refresh">Làm mới</button></div><pre id="cf-log" class="terminal-output cf-log"><?=tms_h($status['log']??'')?></pre></section>
<script>window.TMS_CLOUDFLARE_STATUS_URL='/api/internet-access/status';</script>
