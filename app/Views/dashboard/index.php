...
<section class="dashboard-summary-grid">
 <article class="summary-card"><div class="summary-icon"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M2 12h20"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg></div><div><span>IPv4 LAN</span><strong><?=tms_h($lanIp)?></strong><small><?=tms_h((string)($network['gateway']??'Không phát hiện'))?> gateway</small></div><?php if(filter_var($lanIp,FILTER_VALIDATE_IP)):?><button class="btn btn-secondary btn-small" data-copy="<?=tms_h($lanIp)?>">Copy</button><?php endif;?></article>
...
<section class="two-grid">
 <article class="panel-card"><div class="card-title"><p class="eyebrow">Service Manager</p><h2>Trạng thái dịch vụ</h2></div><div class="service-list"><?php foreach(($services??[]) as $name=>$running):?><div class="service-row"><div><strong><?=tms_h((string)$name)?></strong><span><?=$running?'Tiến trình đang hoạt động':'Không phát hiện tiến trình'?></span></div><b class="status-pill <?=$running?'running':'stopped'?>"><?=$running?'Đang chạy':'Đã dừng'?></b></div><?php endforeach;?></div></article>
	 ...
	 <article class="panel-card"><div class="card-title"><p class="eyebrow">Power Center</p><h2>Điều khiển hệ thống</h2></div><form method="post" action="/service/action" class="action-grid" data-action-form><input type="hidden" name="csrf" value="<?=tms_h((string)($csrf??''))?>"><button class="btn btn-danger-soft" name="action" value="system_reboot" onclick="return confirm('Bạn có chắc chắn muốn khởi động lại thiết bị? (Yêu cầu Root)')">Reboot Android</button><button class="btn btn-danger-soft" name="action" value="system_shutdown" onclick="return confirm('Bạn có chắc chắn muốn tắt nguồn thiết bị? (Yêu cầu Root)')">Shutdown Android</button><button class="btn btn-danger" name="action" value="tms_reset" onclick="return confirm('CẢNH BÁO: Thao tác này sẽ xóa sạch mọi dữ liệu và cấu hình TMS OS. Bạn có chắc chắn?')">Factory Reset TMS</button></form></article>
</section>
