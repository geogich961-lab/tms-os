...
foreach (["\$showShell = true", 'marketplace-grid', 'data-marketplace-install', 'marketplace-modal', 'aria-hidden', 'marketplace.css'] as $needle) {
    if (!str_contains($view, $needle)) { fwrite(STDERR, "Marketplace shell/UI regression: missing {$needle}\n"); exit(1); }
}
foreach (['.marketplace-grid', '@media (max-width: 560px)', '.marketplace-modal-dialog', '.marketplace-card'] as $needle) {
