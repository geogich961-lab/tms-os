...
$baseZip = $root . '/.build/v17.0.18/release/TMS_OS_LATEST.zip';
$releaseZip = $root . '/.build/v17.0.19/release/TMS_OS_LATEST.zip';
$metadataFile = $root . '/.build/v17.0.19/release/RELEASE.json';
...
try {
    expectV1717(is_file($baseZip) && is_file($releaseZip) && is_file($metadataFile), 'Thiếu artifact V17.0.18 hoặc V17.0.19 để mô phỏng nâng cấp.');
    $metadata = json_decode((string)file_get_contents($metadataFile), true);
    expectV1717(is_array($metadata) && ($metadata['version'] ?? '') === '17.0.19', 'RELEASE.json phải khai báo V17.0.19.');
    expectV1717(hash_file('sha256', $releaseZip) === ($metadata['checksum_sha256'] ?? ''), 'Checksum V17.0.19 không khớp RELEASE.json.');

    $archive = new ZipArchive();
    expectV1717($archive->open($releaseZip) === true, 'Không mở được payload V17.0.19.');
    $entries = [];
    ...
    // Bản sửa bảo mật V17.0.19 phải nằm trong payload: default-deny auth + rate limit + CommandRunner.
    ...
    expectV1717(preg_match_all('/\$router->(?:get|post)\(/', $routes) === 136, 'web.php trong payload phải giữ nguyên 136 route.');

    // Backup tự động & offsite V17.0.19 phải nằm trong payload.
    ...
    $base = new ZipArchive();
    expectV1717($base->open($baseZip) === true && $base->extractTo($target), 'Không giải nén được payload V17.0.18.');
    $base->close();
    ...
    $staged = $updates . '/tms-update-v17.0.19.zip';
    expectV1717(copy($releaseZip, $staged), 'Không thể stage ZIP V17.0.19.');
    putenv('HOME=' . $home);
    ...
    expectV1717(!empty($result['ok']) && $service->currentVersion() === 'V17.0.19', 'UpdateService V17.0.18 không áp dụng được V17.0.19.');
    ...
    expectV1717(str_contains($worker, "const VERSION='tms-os-v17.0.19';"), 'Payload chưa làm mới cache Service Worker.');

    echo "PASS: V17.0.18 → V17.0.19 nâng cấp trực tiếp, bảo toàn storage/Cloudflare, có đủ bản sửa kết nối GitHub và giới hạn upload.\n";
} finally {
